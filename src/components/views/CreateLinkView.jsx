    ...
    <div className="w-full flex flex-col items-center text-center animate-fadeIn">
      ...
      <form onSubmit={handleCreate} className="w-full max-w-2xl space-y-4">
        <div className="relative group">
          ...
          <div className="hidden sm:flex relative gap-3 bg-[#0A0A0A] border border-white/10 p-3 rounded-2xl">
              <input type="text" placeholder="Paste your long link here..." className="flex-1 bg-transparent px-6 py-4 text-white placeholder-zinc-600 outline-none text-lg"
          ...
          <div className="sm:hidden flex flex-col gap-4">
              <div className="bg-[#0A0A0A] border border-white/10 rounded-2xl flex items-center shadow-lg">
                  <input type="text" placeholder="Paste your long link here..." className="w-full bg-transparent px-6 py-4 text-white placeholder-zinc-600 outline-none text-lg"
                      value={urlInput} onChange={(e) => setUrlInput(e.target.value)} />
              </div>
              <div className="bg-[#0A0A0A] border border-white/10 rounded-2xl px-6 py-4 flex items-center shadow-lg">
        ...
        <div className="hidden sm:flex justify-center pt-2">
           <div className="bg-white/5 border border-white/10 rounded-xl px-5 py-4 flex items-center w-full max-w-md focus-within:border-indigo-500/50 transition-colors">
              <span className="text-zinc-500 text-base font-mono shrink-0 select-none">linkifyy.netlify.app/</span>
              <input type="text" placeholder="alias (optional)" className="bg-transparent flex-1 text-base text-white outline-none font-mono ml-0.5 placeholder-zinc-700"
                  value={customAlias} onChange={(e) => setCustomAlias(e.target.value.replace(/[^a-zA-Z0-9-_]/g, ''))} maxLength={20}/>
           </div>
        </div>
      </form>
